...
public class AsyncWallets {
     ...
     * The merchant validation proves to Apple that a validated merchant is calling the Apple Pay
     * Javascript APIs.
     * 
     * <p>To integrate Apple Pay via Mollie, you will have to call the Mollie API instead of Apple's API. The
     * response of this
     * API call can then be passed as-is to the completion method, `completeMerchantValidation`.
     * 
     * <p>Before requesting an Apple Pay Payment Session, you must place the domain validation file on your
     * server at:
     * `https://[domain]/.well-known/apple-developer-merchantid-domain-association`. Without this file, it
     * will not be
     * possible to use Apple Pay on your domain.
     * 
     * <p>Each new transaction requires a new payment session object. Merchant session objects are not
     * reusable, and they
     * expire after five minutes.
     * 
     * <p>Payment sessions cannot be requested directly from the browser. The request must be sent from your
     * server. For the
